...
class Dataset(object):
    def __init__(self, dataset_type, net_type):
    ...
    def __next__(self):
        with tf.device('/cpu:0'):
            ...
            if self.net_type == 'tiny':
                batch_label_mbbox = np.zeros((self.batch_size, self.train_output_sizes[0], self.train_output_sizes[0],
                                              self.anchor_per_scale, 5 + self.num_classes))
                batch_label_lbbox = np.zeros((self.batch_size, self.train_output_sizes[1], self.train_output_sizes[1],
                                              self.anchor_per_scale, 5 + self.num_classes))
    ...
    def preprocess_true_boxes(self, bboxes):
        ...
        bboxes_xywh = [np.zeros((self.max_bbox_per_scale, 4)) for _ in range(anchor_size)]
